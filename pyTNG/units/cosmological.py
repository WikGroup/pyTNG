...
class CosmoEquivalence(Equivalence):
    ...
    _dims = (
        cm_length,
        unyt_dims.length,
    )  # we are converting length units in the various cosmological quantities.
...
class CosmoUnits:
    ...
    def __init__(self, scale_factor, littleh):
        ...
        for unit_name in _lut_names:
            ...
            self.registry.add(
                unit_name + "cm", _s, _new_ud, _new_tex, float(_bv), _bool
            )

        # -- adding the little h unit -- #
        self.registry.add(
            "h",
            base_value=self.littleh,
            dimensions=unyt_dims.dimensionless,
            tex_repr="h",
            offset=0.0,
            prefixable=False,
        )
    ...
    def _produce_unit_attributes(self):
        for unit_key, _ in self.registry.lut.items():
            if unit_key != "%cm":
